...
load_data = function(normalise = TRUE, remove_doublets = FALSE, remove_stripped = FALSE, load_corrected = FALSE){
  ...
  counts = readRDS(file.path(data_dir, "raw_counts.rds"))
  genes = read.table(file.path(data_dir, "genes.tsv"), stringsAsFactors = F)
  meta = read.table(file.path(data_dir, "meta.tab"), 
                   header = TRUE, 
                   sep = "\t", 
  ...
  if(normalise){
    sfs = read.table(file.path(data_dir, "sizefactors.tab"), stringsAsFactors = F)[,1]
    sizeFactors(sce) = sfs
  ...
  if(load_corrected){
    corrected = readRDS(file.path(data_dir, "corrected_pcas.rds"))
